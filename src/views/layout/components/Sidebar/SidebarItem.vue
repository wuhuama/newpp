<template>
  <div v-if="!item.hidden" class="menu-wrapper">
    <template v-if="hasOneShowingChild(item.children,item) && (!onlyOneChild.children||onlyOneChild.noShowingChildren)">
      <app-link :to="onlyOneChild.name">
        <el-menu-item :index="onlyOneChild.name" :class="{'submenu-title-noDropdown':!isNest}">
          <item v-if="onlyOneChild.icon&&onlyOneChild.title" :icon="onlyOneChild.icon||(item&&item.icon)" :title="onlyOneChild.title" />
        </el-menu-item>
      </app-link>
    </template>
    ...
    <el-submenu v-else ref="subMenu" :index="item.name" popper-append-to-body>
      <template slot="title">
        <item :icon="item.icon ? item.icon : ''" :title="item.title" />
      </template>
      <sidebar-item
